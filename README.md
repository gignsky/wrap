...
`$ nix run github:gignsky/wrap -- {optional-arguments (i.e. --help)}`

## Installation
### With Nix Shell (temporary installation in current shell)
`$ nix shell github:gignsky/wrap`

### With NixOS (using flakes)
In your `flake.nix` file: 
1. add `inputs.wrap.url = "github:gignsky/wrap";` to your inputs
...
3. add either:
   1. `inputs.wrap.packages.${system}.wrap`
   2. `inputs.wrap.packages.${system}.default`

### Download the binary
1. Navigate to https://github.com/gignsky/wrap/releases
...
1. git clone this repository:
   1. `$ git clone https://github.com/gignsky/wrap.git`
...
4. build binary with justfile (uses nix build command) or build with cargo
   1. `$ just build`
      1. nix build command
      2. produces a binary in `./result/bin/wrap`
   2. `$ cargo build`
      1. cargo build command (debug)
      2. produces a debug binary in `./target/debug/wrap`
   3. `$ cargo build --release`
      1. cargo build command (release)
      2. produces a debug binary in `./target/release/wrap`
...
`$ wrap {optional-arguments (i.e. --help)}`
